...
A vector rewrite of v2: the canvas is no longer the source of truth. Every stroke (brush, position, pressure, tilt, color, seeded RNG state) is recorded as an action; the canvas is just a render of that log. The original UI from v2 is untouched.

Open `v3/index.html` in any modern browser. The previous versions are preserved unchanged (root `index.html` = v1, `v2/index.html` = v2).

---

## What's new in v3

### Vector action log instead of raster snapshots

- **Save format**: `.harmony` files are now JSON action logs (one entry per stroke). A typical session is **a few KB instead of MB** — the canvas raster is never serialized.
- **Local autosave**: persists to `localStorage` (`harmony-v3-actions`) on every commit; quota issues are essentially gone.
- **Deterministic replay**: every brush call goes through a seeded mulberry32 PRNG (`_r()`), seeded per stroke. The whole drawing rebuilds pixel-perfectly from the log.
- **Granular history**: `HISTORY_MAX` jumps from 30 frames to 1000 strokes. Undo/redo just rewinds the index and replays.
- **Free time-travel**: GIF export already renders frames incrementally from the action log — no separate raster history needed.

### Deterministic brushes

- All 50+ brushes were retrofitted to use the seeded `_r()` in place of `Math.random()` (78 call sites in total). No brush authoring changes are needed beyond that one-character substitution.
- Each stroke's seed is `_hashSeed(time, x, y, actions.length)` — different strokes get different streams, but the same stroke replays identically.
...
```jsonc
{
  ...
  "index": 12,         // most-recent committed action (for undo state)
  "actions": [
    {
      "brush": "sketchy",
      "color": [0, 0, 0],
      ...
      "seed": -123456789,
      "reset": true,
      "settings": { "sketchy": { "radius": 63, "density": 1 } },
      "points": [
        {"x": 10,   "y": 10,   "p": 0.5, "tx": 0, "ty": 0},
        {"x": 11.5, "y": 12.2, "p": 0.7, "tx": 5, "ty": -3}
      ]
    },
...
Run with `node v3/smoke-test.js`. The suite covers (in addition to v2's brush-settings consistency checks):

- No raw `Math.random()` in the brush region (must use `_r()`).
- Seeded RNG is deterministic, seed-sensitive, uniform.
- Several heavy-RNG brushes (`splatter`, `sketchy`, `stipple`, `crayon`, `watercolor`, `chalk`, `circles`) produce byte-identical canvas op streams when given the same seed, and diverging streams under different seeds.
- The `.harmony` JSON payload round-trips losslessly.
- `recordStrokeStart`/`recordPoint`/`recordStrokeEnd`/`recordClear`/`replayStroke`/`replayAll`/`exportActionsPayload`/`importActionsPayload` all exist.
- `onMenuDownload` writes JSON; `onMenuUpload` reads JSON.
- Eraser's `destination-out` composite mode doesn't leak into the next brush after a brush switch (the exact corner that motivated the destroy-on-switch path in replay).

---

## What v2 added (preserved)
...
v3 is still one HTML file with everything inlined.

```
v3/
├── index.html      ← the app
├── brushes.json    ← brush-set metadata (carried from v2)
├── changelog.txt   ← version history
├── smoke-test.js   ← `node v3/smoke-test.js`
└── README.md       ← this file
```

The original v1 (`index.html`, `changelog.txt`) at the repo root and v2 (`v2/`) are untouched.
...
# then visit http://localhost:8000/v3/
# tests:
node v3/smoke-test.js
```
...
- Fork & v2/v3: [Barak Bloch](https://github.com/barakbl/harmony)
